...
Aegis is a high-performance security scanning tool for DevOps, CI/CD pipelines, and AI systems. Built in Rust with 670 detection patterns across 34 categories (per `docs/patterns/README.md:3` and `CHANGELOG.md` [0.6.0] entry `660 → 670 patterns`; verified 2026-09-19 against the on-disk pattern corpus).
...
- **Pattern count**: 670 enabled patterns across 34 categories (per `docs/patterns/README.md:3` and `CHANGELOG.md` [0.6.0] `660 → 670 patterns`; verified 2026-09-19). `aegis list` reflects enabled rules only; do not quote counts from memory, since the set is actively pruned.
...
Apache-2.0 (see `LICENSE`, `Cargo.toml:6` and `[workspace.package] license = "Apache-2.0"` at `Cargo.toml:35`).
