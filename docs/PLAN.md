...
- **Detector wired to its caller — DELIVERED (2026-09-24).**
  `aegis scan --detect-clones` (`ScanOptions::detect_clones`) runs the
  detector per analyzed file inside the measured scan window. Pairs are a
  separate output channel — `stats.clones` in JSON (absent when the flag
  is off, so default output stays byte-identical) and a `Code clones`
  human section — never findings: no finding-list, risk-score, SARIF, or
  exit-code impact. A `clone_detection` criterion bench tracks the
...
### Phase 17 — CI lane completeness, enforced gates, release provenance (opened 2026-09-24)

Findings from the 2026-09-24 best-practices comparison against flagship
Rust projects (each claim verified against this tree the day it was
...
- `scripts/release/Dockerfile` builds on `rust:1-bookworm` (floating
  minor) while `docker/Dockerfile` pins `rust:1.88-slim` — same tag,
  different toolchains over time, different binaries. Pin both to the
  same digest or versioned tag.
- `checksums.txt` on the GitHub mirror is verified by
...
**H. Small provenance/CI items — LOW, delivered 2026-09-25.** SHA-pin
...
`docker/Dockerfile` (done 2026-09-24). THIRD-PARTY-NOTICES via
`cargo about` (done 2026-09-25): committed at the repo root as the
shipped artifact, generated by the pinned cargo-about 0.9.2 through
`scripts/release/generate-notices.sh` (whose license `accepted` list
mirrors `deny.toml`'s), carried verbatim in every platform tarball
alongside aegis's own `LICENSE`, and kept fresh by the `notices` CI job
...
`rust-toolchain.toml` (defeats the MSRV job's purpose), `cargo-dist`
(discontinued mid-2025), `release-plz`, GOVERNANCE/MAINTAINERS files,
and a numbered ADR register (six real decisions already live in the
phase history above; backfill into `docs/decisions/` only if a decision
needs to be cited independently).
